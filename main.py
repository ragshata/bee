...
def application():
    ...
    # 0.c decode transport — login нужен дальше + для stat
    try:
    ...
    if device_filter  and ua_device  not in device_filter:
        return early_white("Device",  f"'{ua_device}' not in {device_filter}")
    if os_filter      and ua_os      not in os_filter:
        return early_white("OS",      f"'{ua_os}' not in {os_filter}")
    if browser_filter and ua_browser not in browser_filter:
        return early_white("Browser", f"'{ua_browser}' not in {browser_filter}")
    ...
    try:
        if check_user_agent(ua_raw):
            stats.update({"page":"White","filter":"User-Agent",
                          "descr":"UA in blacklist"})
            click(stats)
            return jsonify(status=1, redirect=1)
    except Exception as ua_e:
        logger.exception(ua_e)
    ...
    try:
        if check_ip(ip):
            stats.update({"page":"White","filter":"IP",
                          "descr":"IP in blacklist"})
            click(stats)
            return jsonify(status=1, redirect=1)
    except Exception as ip_e:
        logger.exception(ip_e)
    ...
    try:
        ...
        country_list   = json_in.get("country_list","").split(",")
        user_country   = check_ip_geo(ip)

        if blocked_method == 1 and user_country in country_list:          # blacklist
            stats.update({"page":"White","filter":"GEO",
                          "descr":"Country in blacklist"})
            click(stats)
            return jsonify(status=1, redirect=1)

        if blocked_method == 2 and user_country not in country_list:      # whitelist
            stats.update({"page":"White","filter":"GEO",
                          "descr":"Country not in whitelist"})
            click(stats)
            return jsonify(status=1, redirect=1)
    except Exception as geo_e:
        logger.exception(geo_e)
        return jsonify(status=0, error_text="Request Failed (06)")

    # 7. VPN / proxy open-ports
    if json_in.get("check_ip", 0) == 1:
        logger.debug("[6] Check open ports")
        try:
            if check_ports(ip):
                stats.update({"page":"White","filter":"VPN/PROXY",
                              "descr":"Anonymizer detected"})
                click(stats)
                return jsonify(status=1, redirect=1)
        except Exception as port_e:
            logger.exception(port_e)
