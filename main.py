...
def application():
    ...
    try:
        if check_user_agent(ua_raw):
            logger.debug("[UA-blacklist] hit -> white")
    ...
    try:
        if check_ip(ip):
            logger.debug("[IP-blacklist] hit -> white")
    ...
    if json_in.get("check_ip") == 1:
        ...
        try:
            if check_ports(ip):
                logger.debug("[VPN] anonymizer detected -> white")
